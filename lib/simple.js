...
END:VCARD
    ...
    conn.sendButton = async (jid, contentText, footer, buffer, buttons, quoted, options) => {
       ...
       conn.sendBut = async(jid, content, footer, button1, row1) => {
	  const buttons = [
...
return await conn.sendMessage(jid, buttonMessage)
   ...
   conn.send2But = async(jid, content, footer, button1, row1, button2, row2) => {
	  const buttons = [
	   { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
...
return await conn.sendMessage(jid, buttonMessage)
   ...
   conn.send3But = async(jid, content, footer,button1, row1, button2, row2, button3, row3) => {
	  const buttons = [
	  { buttonId: row1, buttonText: { displayText: button1 }, type: 1 },
          { buttonId: row2, buttonText: { displayText: button2 }, type: 1 },
  ...
  async sendButtonImg(jid, buffer, content, footer, button1, row1, quoted, options = {}) {
    ...
    async send2ButtonImg(jid, buffer, content, footer, button1, row1, button2, row2, quoted, options = {}) {
      return await this.sendMessage(jid, {
        ...
        headerType: 4,
        imageMessage: (await this.prepareMessageMedia(buffer, MessageType.image, {thumbnail: fs.readFileSync('src/crop_image.jpg')})).imageMessage
      }, MessageType.buttonsMessage, {
        quoted, ...options
      })
    }
